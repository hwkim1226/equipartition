c TO BE COMPILED WITH -fno-automatic FLAG !!!!!! MT 05/05
      PROGRAM king
c
c	Program to make a King model (King1966)
c	
c	Written (thrown together) by d.c.heggie@uk.ac.ed
c       Added velocity dispersion computation by m.trenti@sns.it
...
c     There is a erf function at the end if it is not intrinsic
     ...
     *     NSJ=2*NE+1,NYJ=NE,NYK=M,nstarm = 65536)
	...
	parameter (nmax=10)
      integer myhandler
      external myhandler
	dimension ystart(nmax),coord(nstarm,3),vel(nstarm,3)
c      data scalv/4*0.001d0,1.d0/
      data scalv/5*1.d0/
	external derivs,rkqc
      densty(z) = -sqrt(z)*(z+1.5d0)+0.75*sqrt(pi)*exp(z)*erf(sqrt(z))

      veldis(z) = 3.+24.*(z**2.5)/(10.*sqrt(z)*(3.+2.*z)
     &     -15.*exp(z)*sqrt(pi)*erf(sqrt(z)))
...
c     *     C,NCI,NCJ,NCK,S,NSI,NSJ)
      do j = 1,m
         write (8,100) x(j),(yking(i,j),i=1,2)
      enddo
  100 format (0pf10.5,1p2e10.2)
...
c     &     'certain units):',kevir
     ...
     &     rking, rking/rvir
      ...
      do i = 1,n
         ...
         enddo
         write (10,102) mstar,(coord(i,j),j=1,3),(vel(i,j),j=1,3)
      enddo
c      write (9,*) rking/rvir

c___________________________________________
c     write velocity dispersion at the center of the system
      write(*,*) 'Central velocity dispersion (King & NB units )= ', 
     &     veldis(W0), veldis(W0)/vscale**2
      write(*,*) 'test king66', W0, veldis(W0)/3., vscale
